...
public class GameTest {
	...
	public void gameTest() throws Exception {
		...
		new Company(Location.getLocationByCountry("Deutschland"), "SolarWorld");
		new Company(Location.getLocationByCountry("Deutschland"), "SolarPlus");
		...
		messages.add(new GameDataMessageFromClient("SolarWorld",
				...
				new ProductionFromClient(proOrder), new DistributionFromClient(
						offerList), false, new HumanResourcesFromClient(bBook),
				10000, true));
		...
		ArrayList<GameDataMessageToClient> answer = game
				.startNextRound(messages);
		for (GameDataMessageToClient gm2c : answer) {
			...
			for (RequestToClient req : gm2c.purchase.requests) {
				System.out.println("Angebote für folgende Anfrage: " + req.name
						+ "(" + req.quality + ")");
				for (SupplierOfferToClient off : req.supplierOffers) {
					System.out.println("Angebot: " + off.name + " ("
							+ off.quality + ") Stueckpreis ");
				}
			}
